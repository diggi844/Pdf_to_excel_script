...
MAX_PAGES = None  # TUNE: set to a number (e.g. 3) to test on a subset of pages

# Regex patterns for row classification
# TUNE: confirm these against real CODE values in the PDF.
# Two code schemes have been observed:
#   Alpha-prefixed (ATTIVITA'/PASSIVITA', CONTI D'ORDINE):
...
#   Pure-numeric (COSTI/RICAVI):
#     02        <- level 0 (top-level "parent category") -- EXCLUDED from output
#     0201      <- level 1 header
#     020105    <- level 2 header
#     020105 000004 <- detail row (level-2 code + 6-digit sub-code)
PARENT_CODE_RE = re.compile(r"^([A-Z]{2}|\d{2})$")          # e.g. AA, BB, 02, 01
LEVEL1_CODE_RE = re.compile(r"^([A-Z]{2}\d{2}|\d{4})$")     # e.g. AA01, 0201
LEVEL2_CODE_RE = re.compile(r"^([A-Z]{2}\d{4}|\d{6})$")     # e.g. AA0101, 020105
SUB_CODE_RE = re.compile(r"^\d{6}$")                        # e.g. 000005, 000004

# Page banner line (repeats on every page) — must be excluded entirely,
# it is not a section title and not a data row.
# e.g. "INNOVA   Importi in EURO   Bilancio di esercizio al 31/12/25   pagina 8"
BANNER_RE = re.compile(
    r"(bilancio\s+di\s+esercizio|importi\s+in\s+euro|^pagina\b|\bpagina\s+\d+)",
    re.IGNORECASE,
)
...
# Italian-format amount, e.g. 26.447,61 or 2.160.000,00 or (1.234,56) or 4.043,78-
...
def parse_italian_amount(token: str) -> float:
    ...
    Negative values may appear as a leading '-', wrapped in parentheses,
    e.g. (1.234,56), or with a trailing minus sign, e.g. 4.043,78-."""
    token = token.strip()
    negative = False
...
def classify_and_parse_row(row_words):
    ...
    # - If an amount was found on this line, it's almost certainly a
    #   summary/total line (e.g. "Conti d'ordine attivi  25.987.654,32")
    #   rather than a real data row — ignore it entirely.
    # - If no amount was found, it's likely a wrapped description
    #   continuation — signal it so the caller can append it to the
    #   previous row.
    if amount != 0.0:
        return None
...
def normalize_title_text(tokens):
    """Section titles are sometimes printed with exaggerated letter-spacing,
    e.g. 'C O S T I' instead of 'COSTI', which pdfplumber extracts as
    separate single-character words. Detect that case and rejoin without
    spaces; otherwise join normally (e.g. 'CONTI D'ORDINE')."""
    if tokens and all(len(t) <= 2 for t in tokens):
        return "".join(tokens)
    return " ".join(tokens)


def looks_like_data_row_start(first_token):
    """True if this token could start a real data/header row (has a
    recognizable CODE pattern), meaning it is NOT a section title line."""
    return bool(
        PARENT_CODE_RE.match(first_token)
        or LEVEL1_CODE_RE.match(first_token)
        or LEVEL2_CODE_RE.match(first_token)
    )


def extract_side_rows(words, page_num, default_side_label, previous_side_label=None):
    """Cluster words into rows and classify each one for a single side
    (left or right) of a page.

    The section title (ATTIVITA' / PASSIVITA' / COSTI / RICAVI /
    CONTI D'ORDINE / etc.) is detected dynamically from the first
    non-banner, non-code row on this side of the page, rather than
    assumed fixed. Falls back to previous_side_label (carried over from
    the prior page) if no title is found on this page, then to
    default_side_label as a last resort.

    Returns (results, detected_side_label).
    """
    results = []
    rows = cluster_rows(words)
    side_label = previous_side_label or default_side_label
    ...
    for row_words in rows:
        ...
        # Skip the repeating page banner line entirely (not a title, not data)
        if BANNER_RE.search(full_line):
            continue

        first_token = tokens[0]

        # Before any real data row has been seen, the first non-banner,
        # non-code, non-total/separator row is the section title.
        if not title_captured and not looks_like_data_row_start(first_token):
            if not TOTALS_RE.match(first_token) and not SEPARATOR_RE.match(first_token):
                side_label = normalize_title_text(tokens)
                title_captured = True
                continue
        ...
        title_captured = True  # once real data starts, stop looking for a title

    return results, side_label
...
def process_pdf(input_path, debug=False):
    ...
    with pdfplumber.open(input_path) as pdf:
        for page_index, page in enumerate(pdf.pages):
            ...
            left_rows, last_left_label = extract_side_rows(
                left_words, page_num, "ATTIVITA'", previous_side_label=last_left_label
            )
            right_rows, last_right_label = extract_side_rows(
                right_words, page_num, "PASSIVITA'", previous_side_label=last_right_label
            )

            if debug:
                print(f"\n  --- classified rows: LEFT side = {last_left_label!r}, {len(left_rows)} rows ---")
                for r in left_rows:
                    print(f"    CODE={r['CODE']!r:10} SUB_CODE={r['SUB_CODE']!r:8} "
                          f"AMOUNT={r['AMOUNT']:>15} DESC={r['DESCRIPTION']!r}")
                print(f"\n  --- classified rows: RIGHT side = {last_right_label!r}, {len(right_rows)} rows ---")
                for r in right_rows:
                    print(f"    CODE={r['CODE']!r:10} SUB_CODE={r['SUB_CODE']!r:8} "
                          f"AMOUNT={r['AMOUNT']:>15} DESC={r['DESCRIPTION']!r}")
