...
PARTITA_TOKEN_RE = re.compile(r'^[A-Za-z0-9]+(?:/[A-Za-z0-9]+)+$')    # e.g. "AB123/2025" or "26/2025/FE/2025"
...
def parse_transaction_line(line):
    ...
    # so we don't fix it at exactly 1 token. We cap how far back we reach for it
    # (DOCUMENTO_WORD_MAX_TOKENS) and always leave at least 1 token for Descrizione,
    # since we can't reliably tell prose text apart from a short code token by shape
    # alone. Flagged as an ASSUMPTION — adjust the cap if real data needs more.
    documento_val = ""
    if (len(remainder_tokens) >= 3
        ...
        max_word_tokens = min(DOCUMENTO_WORD_MAX_TOKENS, max(len(before_pair) - 1, 0))
        if max_word_tokens > 0 and before_pair:
            word_tokens = before_pair[-max_word_tokens:]
            doc_word = " ".join(word_tokens)
            documento_val = f"{doc_word} {doc_date} {strip_dots(doc_num_raw)}"
            remainder_tokens = before_pair[:-max_word_tokens]
        else:
            # Not enough tokens left to safely claim a word-prefix without emptying
            # Descrizione entirely; treat Documento as not present.
            documento_val = ""
...
def parse_transaction_blob(blob_text, page_num):
    """
    Parse a multi-line text blob (one pdfplumber "cell" containing several
    transactions joined by newlines) into a list of row dicts, using
    parse_transaction_line() per line, with:
      - missing-SALDO recovery by borrowing from a following orphan line
      - orphan (no-date) lines with amount(s) but no date treated as the page-end
        totals row and skipped entirely
      - per-line error handling: if parsing a given transaction line raises an
        exception, the row is still emitted (with whatever could be salvaged) and
        marked Status="Error"; otherwise Status="Pass".
    Returns (rows, warnings) where warnings is a list of human-readable strings
    flagging anything that needed a guess (missing SALDO, ambiguous DARE/AVERE).
    """
    lines = [l.strip() for l in blob_text.split("\n") if l.strip()]
    rows = []
    warnings = []
    running_saldo = 0.0
    i = 0
    ...
    while i < len(lines):
        ...
        try:
            parsed = parse_transaction_line(lines[i])
            ...
            # SALDO: leave blank if not found (only blank or negative/positive numeric
            # values are valid) rather than forcing 0.
            ...
            warnings.append(f"[page {page_num}] ERROR parsing line '{lines[i][:60]}': {e}")
            rows.append({
                "Page": page_num,
                "Data Comp.": parsed["data"] if parsed else "",
                "Registr. N° del": parsed["registr"] if parsed else "",
                "Descrizione Movimento": parsed["descrizione"] if parsed else lines[i][:100],
...
# --------------------------------------------------------------------------------------
def extract_page(page, page_num, header_state):
    ...
    for settings in table_settings_options:
        ...
        for table in tables:
            for raw_row in table:
                ...
                # Data rows: column 0 holds the blob (per confirmed real structure);
                # but stay defensive in case a row DID split cleanly into columns.
                # DATA_FIELD_COLUMNS excludes the Page/Status columns we add ourselves.
                DATA_FIELD_COLUMNS = [c for c in COLUMNS if c not in ("Page", "Status")]
                non_empty_cells = [c for c in cells if c]
                if len(non_empty_cells) == 1:
                    rows, warnings = parse_transaction_blob(non_empty_cells[0], page_num)
                elif len(non_empty_cells) >= len(DATA_FIELD_COLUMNS) - 2:
                    # Row already split into ~columns; map directly in order.
                    try:
                        ...
                        warnings = [f"[page {page_num}] ERROR mapping split row: {e}"]
                else:
                    # Odd partial split — try parsing the joined text as a blob anyway.
                    rows, warnings = parse_transaction_blob(" ".join(non_empty_cells), page_num)

                all_rows.extend(rows)
                all_warnings.extend(warnings)

        if header_state["seen"] and all_rows:
    ...
    if skipped:
        print(f"  [page {page_num}] skipped {skipped} page-total/carry-forward row(s)")
    for w in all_warnings:
        print(f"  [page {page_num}] WARNING: {w}")
    if not header_state["seen"]:
        print(f"  [page {page_num}] WARNING: column header row not yet found on this page.")
...
def process_pdf(pdf_path, max_pages=None):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for i, page in enumerate(pdf.pages[:pages_to_process], start=1):
            ...
            page_rows = extract_page(page, i, header_state)
