...
- Blank fields (Documento, Partita, missing amounts) are left blank/0 rather than
  guessed.
- Processes only the first 3 pages of the PDF by default (configurable via --pages).

Usage
-----
    python giornale_to_excel.py input.pdf output.xlsx
    python giornale_to_excel.py input.pdf output.xlsx --pages 3   (default: first 3 pages)
    python giornale_to_excel.py input.pdf output.xlsx --pages 0   (process ALL pages)
...
PARTITA_TOKEN_RE = re.compile(r'^[A-Za-z0-9]+/\d{4}$')        # e.g. "AB123/2025"
...
def parse_transaction_line(line):
    ...
    # Documento N° del e Prot.: trailing 3-token pattern [word][date][docnum].
    documento_val = ""
    if (len(remainder_tokens) >= 3
            and DATE_TOKEN_RE.match(remainder_tokens[-2])
            and DOCNUM_TOKEN_RE.match(remainder_tokens[-1])):
        doc_word, doc_date, doc_num_raw = remainder_tokens[-3], remainder_tokens[-2], remainder_tokens[-1]
        documento_val = f"{doc_word} {doc_date} {strip_dots(doc_num_raw)}"
        remainder_tokens = remainder_tokens[:-3]
...
def parse_transaction_blob(blob_text):
    """
    ...
    while i < len(lines):
        ...
        importo, saldo = None, None

        if len(amounts) >= 2:
            importo, saldo = amounts[0], amounts[-1]
        elif len(amounts) == 1:
            importo = amounts[0]
            ...
            if i + 1 < len(lines):
                ...
                if next_is_orphan:
                    ...
                    if next_amounts:
                        saldo = parse_it_number(next_amounts[0])
                        warnings.append(
                            f"Row '{parsed['data']} {parsed['descrizione'][:40]}': SALDO was "
                            f"missing, borrowed {saldo} from following line — please verify."
                        )
                        i += 1  # consume that orphan line

        dare, avere, ambiguous = assign_dare_avere(importo or 0, saldo, running_saldo)
        if ambiguous:
            warnings.append(
                f"Row '{parsed['data']} {parsed['descrizione'][:40]}': no SALDO available "
                f"to infer DARE vs AVERE — defaulted to DARE, please verify."
            )
        ...
        for table in tables:
            for raw_row in table:
                ...
                if len(non_empty_cells) == 1:
                    rows, warnings = parse_transaction_blob(non_empty_cells[0])
                elif len(non_empty_cells) >= len(COLUMNS) - 2:
                    # Row already split into ~columns; map directly in order.
                    row = {col: "" for col in COLUMNS}
                    for col, val in zip(COLUMNS, non_empty_cells):
                        row[col] = val
                    row["DARE"] = parse_it_number(row.get("DARE", "")) or 0
                    row["AVERE"] = parse_it_number(row.get("AVERE", "")) or 0
                    row["SALDO"] = parse_it_number(row.get("SALDO", "")) or 0
                    rows, warnings = [row], []
                else:
                    # Odd partial split — try parsing the joined text as a blob anyway.
                    rows, warnings = parse_transaction_blob(" ".join(non_empty_cells))
...
def main():
    ...
    parser.add_argument("--pages", type=int, default=3,
                         help="Number of pages to process from the start of the PDF (default: 3). "
                              "Use 0 to process all pages.")
