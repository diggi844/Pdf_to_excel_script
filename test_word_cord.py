...
# The columns we actually extract data into (excludes Page/Status, which we compute).
...
def find_header_zones(page):
    ...
    Returns (zones, header_top) where zones is a list of (col_name, x_start, x_end)
    covering the full page width left-to-right, or (None, None) if the header
    row isn't found or its words can't be confidently matched to all columns.
    """
    try:
    ...
    for top, line_words in cluster_words_into_lines(words):
        cell_texts = [w["text"] for w in line_words]
        if not is_header_row(cell_texts):
            continue

        zones = _match_header_words_to_zones(line_words)
        if zones:
            return zones, top
...
def _match_header_words_to_zones(line_words):
    ...
    Returns None if the expected words can't all be matched (falls back to the
    regex/blob approach for this page elsewhere).
    """
    remaining = list(line_words)
    col_ranges = []
    ...
    for col_name in DATA_FIELD_COLUMNS:
        ...
        for exp_word in expected:
            if not temp or _normalize_word(temp[0]["text"]) != _normalize_word(exp_word):
                ok = False
                break
            matched.append(temp.pop(0))

        if not ok or not matched:
            return None  # couldn't confidently match this column's header text
...
def bin_words_into_zone_rows(page, zones, header_top, page_num):
    """
...
def convert_zone_row(row, page_num):
    ...
    dict, with Status="Pass"/"Error" depending on whether conversion succeeded.
    ...
    try:
        dare = parse_it_number(row.get("DARE", "")) or 0
        avere = parse_it_number(row.get("AVERE", "")) or 0
        saldo_raw = row.get("SALDO", "").strip()
        saldo = parse_it_number(saldo_raw) if saldo_raw else ""
...
def process_pdf(pdf_path, max_pages=None):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for i, page in enumerate(pdf.pages[:pages_to_process], start=1):
            ...
            zones, header_top = find_header_zones(page)
